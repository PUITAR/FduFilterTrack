...
*/
// 用来build、保存、载入索引，search、delete、insert point
class AbstractIndex
{
  public:
    ...
    virtual void build(const std::string &data_file, const size_t num_points_to_load,
                       IndexBuildParams &build_params, const std::string& label_file) = 0;
    ...
    virtual void save(const char *filename, std::string label_filename, bool compact_before_save = false) = 0;
  ...
  private:
    ...
    virtual std::pair<uint32_t, uint32_t> _search(const DataType &query, const size_t& K, const uint32_t& L, 
                                                  const uint32_t& threshold_1, const uint32_t& threshold_2, 
                                                  const int32_t& label1, const int32_t& label2, std::any &indices, float *distances = nullptr) = 0;
    virtual std::pair<uint32_t, uint32_t> _search_with_filters(const DataType &query, const std::string &filter_label,
                                                               ...
                                                               const uint32_t threshold_1, const uint32_t threshold_2) = 0;
