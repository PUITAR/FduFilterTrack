...
/*
    把label转换成int，并将转换后的写入至outFileName，把{label,ids}写入至mapFileName
*/
...
#ifndef EXEC_ENV_OLS
inline size_t get_graph_num_frozen_points(const std::string &index_path)
{
    // size_t expected_file_size;
    // uint32_t max_observed_degree, start;
    uint32_t file_frozen_pts;
    ...
    in.open(index_path + "_csr_offset.bin", std::ios::binary);
    in.read((char *)&file_frozen_pts, sizeof(uint32_t));
...
inline std::vector<std::string> read_file_to_vector_of_strings(const std::string &filename, bool unique = false)
{
    ...
    if (filename != "")
    {
        ...
        if (file.fail())
        {
            throw diskann::ANNException(std::string("Failed to open file 2") + filename, -1);
        }
        std::string line;
...
inline std::vector<std::vector<LabelT>> read_file_to_vector_of_num(const std::string &filename,
                                                                        bool unique = false)
{
    std::vector<std::vector<LabelT>> result;
    if (filename != "")
    {
        std::ifstream file(filename, std::ios::binary);
        if (file.fail())
        {
            throw diskann::ANNException(std::string("Failed to open file 3") + filename, -1);
        }
        int64_t nrow;
        int64_t ncol;
        int64_t nnz; // 稀疏矩阵中的非零元素的总数
        file.read((char *)&nrow, sizeof(int64_t));
        file.read((char *)&ncol, sizeof(int64_t));
        file.read((char *)&nnz, sizeof(int64_t));
        ...
        file.read((char *)indptr, (nrow + 1) * sizeof(int64_t));

        int32_t *indices = new int32_t[nnz];
        file.read((char *)indices, nnz * sizeof(int32_t));
