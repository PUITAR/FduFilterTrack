...
template <typename T, typename LabelT = uint32_t> class PQFlashIndex
{
  ...
  private:
    ...
    // data info
    uint64_t num_points = 0; // pq_file_points_num
    uint64_t num_frozen_points = 0;
    uint64_t frozen_location = 0;
    uint64_t data_dim = 0; // pq_file_dim
    ...
    tsl::robin_map<uint32_t, std::pair<uint32_t, uint32_t *>> nhood_cache; // 节点id，{邻居数量和邻居节点数组}

    // coord_cache
    T *coord_cache_buf = nullptr;
    tsl::robin_map<uint32_t, T *> coord_cache; // 节点 ID + vector
