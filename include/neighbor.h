...
class NeighborPriorityQueue
{
  public:
    ...
    bool insert(const Neighbor &nbr) // 插入后从小到大排列
    {
        if (_size == _capacity && _data[_size - 1] < nbr) // 如果满了且距离比最大的还大，则不插入
        {
            return false;
        }
        ...
        if (lo < _cur) 
        {
            _cur = lo; // cur表示最近被插入的node的最小索引
        }
        return true;
    }

    // 查找并返回在集合中最接近但尚未扩展（expanded）的 Neighbor 实例
    ...
    void remove_by_idx(size_t idx) {
        if (idx < _size - 1)
        {
            std::memmove(&_data[idx], &_data[idx+1], (_size - idx - 1) * sizeof(Neighbor));
        }
        --_size;
    }
