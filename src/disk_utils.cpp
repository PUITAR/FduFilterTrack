...
// 将多个子分区的Vamana索引合并成一个大的Vamana索引
// 如果有filter，则将{label, medoids}存到labels_to_medoids_file，形式是{label, medoid1, medoid2...}
                 ...
                 const std::string &labels_to_medoids_file)
{
    ...
    // find max node id
    size_t nnodes = 0; // 找到所有节点的最大ID（nnodes）
    size_t nelems = 0; // 节点个数
    ...
    // 创建一个node_shard向量，将节点映射到它们所属的子分区，以便后续节点合并时能够快速查找其所属的子分区。{nodeid, shardid}
    ...
    }); // 根据nodeid升序排序,可能存在同一个node在两个shard里，所以还要根据shardid升序排序
    diskann::cout << "Finished computing node -> shards map" << std::endl;

    // will merge all the labels to medoids files of each shard into one combined file
    if (use_filters)
    {
        std::unordered_map<uint32_t, std::vector<uint32_t>> global_label_to_medoids; // 存储{label, medoids}
    ...
    // create cached vamana readers，也就是之前为每个聚类建立的索引
    ...
    for (auto &reader : vamana_readers)
    {
        ...
        reader.read((char *)&input_width, sizeof(uint32_t)); // 索引的R
    ...
    std::ofstream medoid_writer(medoids_file.c_str(), std::ios::binary); // 存放每个索引的_start节点
    ...
    // 遍历所有子分区，读取每个子分区的Vamana索引文件，获取其medoid节点（索引的中心节点）的ID并将其重命名（防止节点id相同）。
    // 写入至medoid_writer，并将最后一个索引的medoid节点写入merged_vamana_writer
    for (uint64_t shard = 0; shard < nshards; shard++)
    {
        uint32_t medoid;
        // read medoid
        vamana_readers[shard].read((char *)&medoid, sizeof(uint32_t)); // 索引的_start节点，具体可以看save函数是怎么存索引的
        ...
        if (shard == (nshards - 1)) //--> uncomment if running hierarchical
            merged_vamana_writer.write((char *)&medoid, sizeof(uint32_t)); // 把最后一个索引的medoid写入至merged_vamana
    }
    ...
    for (const auto &id_shard : node_shard)
    {
        ...
        if (cur_id < node_id) // 其实这里是cur_id != node_id，表示cur_id的相关邻居已经处理完了，将其写入至merged_vamana_writer中
        {
            // Gopal. random_shuffle() is deprecated.
        ...
        vamana_readers[shard_id].read((char *)&shard_nnbrs, sizeof(uint32_t)); // 读取节点的邻居
...
// 将密集标签的数据点进行拆分，以限制每个数据点拥有的标签数量，从而生成新的数据点，并将拆分后的数据写回到输出文件。
// out_labels_file存储所有节点（旧的+新的）的label(int)
// out_metadata_file存储新id与旧id的pair
// out_data_file存储data_file原先的向量，并将生成的新的节点的向量（即对应原始节点的向量）添加到后面
template <typename T>
                          ...
                          const std::string out_metadata_file)
{
    ...
    std::unordered_map<uint32_t, uint32_t> dummy_pt_ids; // 记录新生成的数据点与原始数据点之间的映射关系{新id，旧id}。
    uint32_t next_dummy_id = (uint32_t)npts;

    uint32_t point_cnt = 0; // 追踪当前正在处理的数据点的编号

    std::vector<std::vector<uint32_t>> labels_per_point; // {id, label数组(int)}
    labels_per_point.resize(npts);

    uint32_t dense_pts = 0; // 拆分的数据点的总数
    if (labels_stream.is_open())
    {
        while (getline(labels_stream, line))
        {
            ...
            while (getline(iss, token, ','))
            {
                if (lbl_cnt == density)
                {
                    if (label_host == point_cnt) // 如果原始节点第一次拆分
                        dense_pts++;
...
{ // assumes ith row is for ith point in labels file
...
// k-means分区，并将node分配至2个聚类中，并为每个聚类构建图
// 最后将图合并成一个大图
                              ...
                              const uint32_t Lf)
{
    ...
    // 内存够用就使用build_memory_index
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) // ram_budget == M(memory limit while indexing)
    {
    ...
    for (int p = 0; p < num_parts; p++)
    {
        ...
        // 把对应Id的节点的vector写入shard_base_file
        ...
        if (!use_filters)
        {
            _index.build(shard_base_file.c_str(), shard_base_pts, paras); // 为该shard构件图
        }
        else
        {
            diskann::extract_shard_labels(label_file, shard_ids_file, shard_labels_file); // 从label_file中提取出shard_ids_file节点的label，存在shard_labels_file
            if (universal_label != "")
            { //  indicates no universal label
                LabelT unv_label_as_num = 0;
                _index.set_universal_label(unv_label_as_num);
...
// 将Vamana索引数据重新组织并写入到磁盘文件中，最后将向量和邻居都存储在output_file中
template <typename T>
                        ...
                        const std::string reorder_data_file)
{
    ...
    base_reader.read((char *)&ndims, sizeof(uint32_t)); // 向量维度
    ...
    vamana_reader.read((char *)&width_u32, sizeof(uint32_t)); // R
    ...
    max_node_len = (((uint64_t)width_u32 + 1) * sizeof(uint32_t)) + (ndims_64 * sizeof(T)); // 每个节点的存储大小
    nnodes_per_sector = SECTOR_LEN / max_node_len; // 每个扇区能存储的节点数
    ...
    /*
    对于每个扇区，它从内存中的Vamana索引文件中读取节点的邻居信息，以及节点的坐标信息，将这些数据存储在 node_buf 中，并逐个节点地写入 sector_buf 中。
    在完成一个扇区的数据写入后，将 sector_buf 中的数据刷新到磁盘文件。
    */
...
// 生成原始向量经过PQ压缩的文件（包括向量和pivot）
// 生成聚类后的多个索引文件以及将多个索引合并的单个索引文件，单个索引文件存在_mem.index，多个索引文件就删了
// 最后将索引数据重新组织并写入到磁盘文件中，存在_disk.index（索引+向量，向量是压缩或者原始向量）
// 如果是filter，还存了universal_label.txt labels_to_medoids.txt 倒排索引.txt
template <typename T, typename LabelT>
                     ...
                     const uint32_t Lf)
{
    ...
    {
        disk_pq_dims = atoi(param_list[5].c_str()); // PQ bytes for disk index
        use_disk_pq = true;
        if (disk_pq_dims == 0)
            use_disk_pq = false;
    }
    ...
    double final_index_ram_limit = get_memory_budget(param_list[2]); // B (RAM limit of final index in GB)
    ...
    double indexing_ram_budget = (float)atof(param_list[3].c_str()); // M (memory limit while indexing)
    ...
    if (use_disk_pq)
    {
        generate_disk_quantized_data<T>(data_file_to_use, disk_pq_pivots_path, disk_pq_compressed_vectors_path,
                                        compareMetric, p_val, disk_pq_dims); // 为后续将vector和index存储到一起做准备
    }
    size_t num_pq_chunks = (size_t)(std::floor)(uint64_t(final_index_ram_limit / points_num)); 
    // 用B来限定压缩的维度(num_pq_chunks)
    // 如果还制定了QD参数，那么就用QD
    num_pq_chunks = num_pq_chunks <= 0 ? 1 : num_pq_chunks;
    num_pq_chunks = num_pq_chunks > dim ? dim : num_pq_chunks;
    num_pq_chunks = num_pq_chunks > MAX_PQ_CHUNKS ? MAX_PQ_CHUNKS : num_pq_chunks;
    ...
    generate_quantized_data<T>(data_file_to_use, pq_pivots_path, pq_compressed_vectors_path, compareMetric, p_val,
                               num_pq_chunks, use_opq, codebook_prefix); // 为后续search工作准备
...
// This code should only be available in the "build" DLL.
    ...
    diskann::build_merged_vamana_index<T, LabelT>(data_file_to_use.c_str(), diskann::Metric::L2, L, R, p_val,
                                                  ...
                                                  labels_to_medoids_path, universal_label, Lf); // 用的是原始向量
    ...
    gen_random_slice<T>(data_file_to_use.c_str(), sample_base_prefix, sample_sampling_rate); // 用来给search用
    if (use_filters)
    {
