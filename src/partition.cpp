...
// 从给定的输入数据矩阵中按照指定的采样概率 p_val 随机选取样本，生成一个采样后的子集，并将子集数据存储在 sampled_data 中（数据的维度不变）
// 结果存在output_prefix文件中
template <typename T>
...
// 将数据样本分配给最近的k_base个聚类中心，将大型数据集划分成多个子分区，并生成每个子分区的ID映射文件
// ID = block_i * block_size + 节点block内的索引 
template <typename T>
...
int retrieve_shard_data_from_ids(const std::string data_file, std::string idmap_filename, std::string data_filename)
{
    ...
    for (size_t block = 0; block < num_blocks; block++) // 提取出向量，找到对应id
    {
        size_t start_id = block * block_size;
        size_t end_id = (std::min)((block + 1) * block_size, num_points);
        size_t cur_blk_size = end_id - start_id;
...
// 对数据进行K-means聚类，并使得最大的聚类的RAM使用量小于RAM limit，并将pivot保存在文件中
// 并将节点分配至k_base个聚类中心，最后保存到num_centers个文件中
// k_base == 2，也就是论文中将向量分配至两个pivot
template <typename T>
                              ...
                              size_t graph_degree, const std::string prefix_path, size_t k_base)
{
    ...
    while (!fit_in_ram)
    {
        ...
        /* 估算每个聚类中的包含的测试数据大小，即计算每个聚类的测试数据样本的数量。
        对于每个test node，找到距离其最近的k_base个聚类中心（pivot）。对应的，该向量就属于k_base个聚类中心
        这些数据点将被认为是属于该聚类的测试数据样本。
        这个函数最终将估算的测试数据在每个聚类中的大小存储在cluster_sizes向量中
        */
        ...
        for (auto &p : cluster_sizes)
        {
            ...
            // testing sample.
            // 为了避免内存使用估算的误差，通过sampling_rate（采样率）来估算测试数据在该聚类中的实际大小
            // 然后得到最大的聚类所使用的RAM
        ...
        /*
        如果最大的聚类所使用的RAM使用量超过了指定的RAM预算，函数会增加num_parts(聚类中心)的数量，
        并重新执行k-means聚类步骤和RAM使用量估算，直到所有分区都可以在指定的RAM预算内。
        */
