...
void aggregate_coords(const uint32_t id, const uint8_t *all_coords, const size_t ndims, uint8_t *out)
{
    memcpy(out, all_coords + id * ndims, ndims * sizeof(uint8_t));
}

void pq_dist_lookup(const uint8_t *pq_ids, const size_t n_pts, const size_t pq_nchunks, const float *pq_dists,
                    std::vector<float> &dists_out)
{
    // pq_ids存每个向量每个chunk的中心点id
    // pq_dists存每个chunk中心点id到其他中心点的距离
...
// 在每个块上运行 K-means 算法，计算出 PQ 的质心（pivots），数量是num_centers个
                       ...
                       bool make_zero_mean)
{
    ...
    if (file_exists(pq_pivots_path)) // 如果文件存在，获取文件中保存的聚类中心的维度和数量
    {
        ...
        if (file_dim == dim && file_num_centers == num_centers)
        {
            ...
            // 比较加载的聚类中心维度 file_dim 是否与当前需要生成的聚类中心维度 dim 相等，并比较聚类中心数量 file_num_centers 是否与当前需要生成的聚类中心数量 num_centers 相等。
            // 如果维度和数量都相等，则输出一条提示信息，并返回 -1，表示不需要重新生成聚类中心。
            return -1;
        }
    }
...
#pragma omp parallel for schedule(static, 65536)
    ...
    /*
    创建一个 cumul_bytes 向量，用于存储每个部分数据在文件中的累积字节偏移量。初始时，将向量的前三个元素设为 0，分别用于存储元数据、聚类中心数据和块偏移数据的字节偏移量。
    将元数据保存到文件，并更新 cumul_bytes[0] 为保存元数据后的累积字节偏移量。
    将聚类中心数据保存到文件，并更新 cumul_bytes[1] 为保存聚类中心数据后的累积字节偏移量。
    将块偏移数据保存到文件，并更新 cumul_bytes[2] 为保存块偏移数据后的累积字节偏移量。
    将 cumul_bytes 向量中的内容（各部分数据在文件中的累积字节偏移量）保存到文件中。
    输出保存完成的信息，包括保存的文件路径、总大小（即最后一个元素 cumul_bytes[cumul_bytes.size() - 1] 的值）。
    */
...
// 通过质心将原始数据编码，并将结果存储在 pq_compressed_vectors_path 文件中
template <typename T>
                                 ...
                                 bool use_opq)
{
    ...
    size_t block_size = num_points <= BLOCK_SIZE ? num_points : BLOCK_SIZE; // 将大型文件或矩阵分解为较小的数据块
...
// 生成聚类中心，并存储在pq_pivots_path
// 根据中心对原始数据进行编码，存储在pq_compressed_vectors_path
template <typename T>
