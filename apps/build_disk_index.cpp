...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // B / points_num == num_pq_chunks
        // 该参数是用来决定压缩原始向量的维度的，并保存在文件里，在search_disk_index会用到
        ...
        required_configs.add_options()("build_DRAM_budget,M", po::value<float>(&M)->required(),
                                       "DRAM budget in GB for building the index"); // 构建索引时的DRAM内存预算，单位为GB
        ...
        // 和参数B的作用一样，如果指定了QD参数且该参数合法，则使用QD决定维度；否则用B
        ...
        optional_configs.add_options()("codebook_prefix", po::value<std::string>(&codebook_prefix)->default_value(""),
                                       "Path prefix for pre-trained codebook"); // PQ算法的pivot
        
        // 这个参数会影响use_disk_pq这个变量，该变量为真，则create_disk_layout的时候，_disk.index文件里存的就是压缩向量（维度是PQ_disk_bytes）和邻居
        // 这个参数不会影响构建索引的压缩向量的维度，也不会影响search工作
        ...
        // 在索引中包含完整精度的数据。必须与SSD上的压缩数据(也是针对_disk.index的，如果想了解的话看create_disk_layout函数)一起使用
        optional_configs.add_options()("append_reorder_data", po::bool_switch()->default_value(false), 
                                       "Include full precision data in the index. Use only in "
                                       "conjuction with compressed data on SSD."); 
        // 在内存中构建多个分区索引时，将原始向量划分为多少个子空间，如果为0则不压缩
        // 这个是在构建索引时决定压缩向量维度的。而QP和B都是为了search工作准备的，在构建索引时用不到
        ...
        optional_configs.add_options()("filter_threshold,F", po::value<uint32_t>(&filter_threshold)->default_value(0),
                                       ...
                                       "internally where each node has a maximum F labels."); // 用于控制将现有节点拆分以生成新的图形，其中每个节点具有最大 F 个标签。
