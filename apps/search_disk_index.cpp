                      ...
                      const std::vector<std::string> &query_filters, const bool use_reorder_data = false)
{
    ...
    std::string warmup_query_file = index_path_prefix + "_sample_data.bin"; // 抽样的数据，是在build_disk_index的最后创建的，向量没有压缩过
    ...
    if (num_nodes_to_cache > 0) // 在sample_bin中找出最频繁访问的num_nodes_to_cache个节点
        _pFlashIndex->generate_cache_list_from_sample_queries(warmup_query_file, 15, 6, num_nodes_to_cache, num_threads,
                                                              ...
                                                              std::string sample_bin, uint64_t l_search,
                                                                      ...
                                                                      std::vector<uint32_t> &node_list
                                                              */
    _pFlashIndex->load_cache_list(node_list); // 加载node_list中的节点、节点vector及其邻居数量和邻居ids
    node_list.clear();
        ...
        for (int64_t i = 0; i < (int64_t)query_num; i++)
        {
            ...
            else
            {
                ...
                { // one label for all queries
                    label_for_search = _pFlashIndex->get_converted_label(query_filters[0]); // 这个输入的是字符串的label，程序会将其转换成对应的int型
                }
                else
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        optional_configs.add_options()("beamwidth,W", po::value<uint32_t>(&W)->default_value(2),
                                       program_options_utils::BEAMWIDTH); // 一次从扇区内取多少个节点的邻近节点
        optional_configs.add_options()("num_nodes_to_cache", po::value<uint32_t>(&num_nodes_to_cache)->default_value(0),
                                       program_options_utils::NUMBER_OF_NODES_TO_CACHE); // Number of BFS nodes around medoid(s) to cache
