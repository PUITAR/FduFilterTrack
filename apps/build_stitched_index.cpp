 ...
 */
// 将所有标签的图进行合并,并将随机选取每个标签的entry数据点
template <typename T>
    ...
    tsl::robin_map<std::string, std::vector<uint32_t>> label_id_to_orig_id_map)
{
    ...
    for (const auto &lbl : all_labels) 
    {
        ...
        std::tie(curr_label_index, curr_label_index_size) =
            diskann::load_label_index(curr_label_index_path, labels_to_number_of_points[lbl]); // 遍历每个标签，读取对应的索引文件
        curr_label_entry_point = (uint32_t)random(0, curr_label_index.size()); // 随机选择一个点作为entry
        label_entry_points[lbl] = label_id_to_orig_id_map[lbl][curr_label_entry_point];

        for (uint32_t node_point = 0; node_point < curr_label_index.size(); node_point++) // 遍历该索引中的点
        {
            uint32_t original_point_id = label_id_to_orig_id_map[lbl][node_point];
            for (auto &node_neighbor : curr_label_index[node_point]) // 遍历当前点的邻居
            {
                ...
                if (std::find(curr_point_neighbors.begin(), curr_point_neighbors.end(), original_neighbor_id) ==
                    curr_point_neighbors.end()) // 如果该邻居没有加入到最终的图，则加进去
                {
...
int main(int argc, char **argv)
{
    ...
    if (data_type == "uint8")
        label_id_to_orig_id_map = diskann::generate_label_specific_vector_files<uint8_t>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels, all_labels); // {label, ids}
